...
@section('title', 'Sign In - Le Nium Legal')
    ...
    <div class="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <form action="{{ route('login.verify') }}" method="POST" class="space-y-5">
            ...
            <div class="flex items-center justify-between pt-1">
                ...
                <a href="{{ route('password.request') }}" class="text-sm text-blue-600 hover:text-blue-700 font-semibold">Forgot Password?</a>
            </div>
        ...
        <div class="border-t border-gray-200 px-6 py-6 bg-gray-50">
            <p class="text-center text-sm text-gray-600">Don't have an account? <a href="{{ route('register') }}" class="text-blue-600 font-semibold hover:text-blue-700">Create one</a></p>
        </div>
    </div>
</div>
...
<script>
    ...
    /*
    const loginForm = document.querySelector('form[action="{{ route('login.verify') }}"]');
    if (loginForm && '{{ config('services.recaptcha.site_key') }}') {
