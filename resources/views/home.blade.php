...
@section('title', 'Le Nium Legal - Remote Guidance & Support Services')
...
<script>
    ...
    document.getElementById('signupForm')?.addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch('{{ route('auth.register.initial') }}', {
                method: 'POST',
                headers: {
    ...
    document.getElementById('verifyCodeBtn')?.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('{{ route('auth.register.verify') }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
            ...
            if (response.ok) {
                ...
                window.location.href = '{{ route('account.created') }}';
            } else {
                const error = await response.json();
